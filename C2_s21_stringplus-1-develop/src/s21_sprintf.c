...
int s21_sprintf(char *str, const char *format, ...) {
  int n = 0;  // индекс символа в результирующей строке
  struct_format form;
  va_list arg;
  va_start(arg, format);
  ...
  for (int i = 0; i < format_length; i++) {
    if (format[i] == '%') {
      ...
      form.length_h = -1;
      form.length_l = -1;
      form.zero = -1;  // для нуля после точки
      int flag = 1;  // flag=0, если встречен спецификатор
      while (flag) {
        i++;
        if (format[i] == '-') {
          form.minus = 1;
          ...
          form.length_l = 1;
          /*if (format[i + 1] == 'l') {// случай ll для lon long double, в общей
          беседе пишут, что этот случай не надо рассматривать form.length_l = 2;
          }*///далее этот случай закомменчен, чтобы повысить покрытие тестами
    ...
    } else {  // если встречен простой текст
      str[n] = format[i];
      n++;
    }
  }
...
int length_of_accuracy_or_width(const char *format, int *i) {
  ...
  if (*(format + (*i)) >= '0' && *(format + (*i)) <= '9') {
    length = 0;
    for (int y = 0; *(format + (*i)) >= '0' && *(format + (*i)) <= '9'; y++) {
      length = (length * 10) + *(format + (*i)) - '0';
      (*i)++;
    }
  }
  (*i)--;
...
int check_specificator(char *str, int *n, struct_format form, va_list arg) {
  ...
  } else if (form.specifier == '%') {  // для случая, когда два процента подряд
...
void write_c(char *str, int *n, struct_format form, va_list arg) {
  if (form.length_l == 1) {  // если широкий символ
    wchar_t arg_char_l = va_arg(arg, wchar_t);
    write_wide_char(str, n, form, arg_char_l);
  } else {
    char arg_char = va_arg(arg, int);  // int, потому что при char выдает ошибку
...
void write_char(char *str, int *n, struct_format form, char arg) {
  if (form.minus == -1 &&
      form.width != -1) {  // нет минуса, но есть ширина, то тогда сначала
                           // печатаем пробелы, потом символ
    write_space(str, n, 1, form);
  }
  str[*n] = arg;
  (*n)++;
  if (form.minus == 1 &&
      form.width != -1) {  // если есть выравнивание по левому краю
    write_space(str, n, 1, form);
  }
}
...
void write_wide_char(char *str, int *n, struct_format form, wchar_t arg) {
  ...
  memory = (char *)calloc(
      2, sizeof(wchar_t));  // заменил arg на 2, так как для строки из одного
                            // символа типа wchar_t нужен
  if (memory == s21_NULL) {  // массив длиной 2 = сам символ + '\0'
    exit(0);
  }
  wctomb(memory, arg);  // переводим широкое число в символьную строку
  *(memory + s21_strlen(memory)) = '\0';
  ...
  int i = 0;
  while (memory[i] != '\0') {  // после вывода нужного количества пробелов,
                               // печатаем символьную строку из широкого символа
    str[*n] = memory[i++];
    (*n)++;
  }
  if (form.minus == 1 &&
...
void write_f(char *str, int *n, struct_format form, va_list arg) {
  ...
  if (form.accuracy == 1 && form.accuracy_length == 0) {
    if (length_int_kost > length_int) length_int = length_int_kost;
    full_len = length_int;
  } else {
  ...
  int sign_len = count_sign_len(form, (long long)arg_f);
  if (form.minus == 1) {
    if (signbit(arg_f)) {
      str[*n] = '-';
      (*n)++;
...
void write_s(char *str, int *n, struct_format form, va_list arg) {
  if (form.length_l == 1) {  // если строка широких символов
    ...
    if (arg_s == s21_NULL) {
      arg_s = "(null)";  // пробовал выводить нулевой указатель, получалась
...
void write_str(char *str, int *n, struct_format form, char *arg_s) {
  ...
  if (form.minus ==
      1) {  // если есть минус, то сначала выводим строку, потом пробелы
...
void write_str_l(char *str, int *n, struct_format form, wchar_t *arg_s_l) {
  ...
  if (form.accuracy_length != -1) {  // если после точки задана точность
    if (full_len >
        form.accuracy_length) {  // если точность меньше длины строки, то все
                                 // равно печатаем всю строку
...
void write_int(char *str, int *n, struct_format form, long long arg) {
  ...
  if (form.accuracy == 1 && form.accuracy_length == 0 && arg == 0 &&
      form.width != -1) {
  ...
  if (arg != 0 ||
      (form.accuracy = 1 && form.accuracy_length != 0 && arg == 0)) {
    if (form.minus == 1) {
      if (arg < 0) {
        str[*n] = '-';
        (*n)++;
  ...
  } else if (form.accuracy != -1 && form.accuracy_length == 0 && arg == 0 &&
             form.space != -1) {
    str[*n] = ' ';
    (*n)++;
  }
...
int count_sign_len(struct_format form, long long arg) {
  int sign_len = 0;
  if ((form.space == 1 && arg >= 0) || (form.plus == 1 && arg >= 0) ||
      arg < 0) {
    if (form.width != -1) {
      sign_len = 1;
    }
...
void write_space(char *str, int *n, int length, struct_format form) {
  /*if (form.zero == 1 && form.accuracy_length == -1) {// так как не
  рассматриваем флаг ноль for (int i = 0; i < (form.width - length); i++) {
          str[*n] = '0';
          (*n)++;
      }
  } else {*/
  for (int i = 0; i < (form.width - length);
       i++) {  // печатаем пробелы для заданной ширины
    str[*n] = ' ';
    (*n)++;
  }
  //}
...
int len_of_wchar(wchar_t *arg_s_l) {  // определяет длину строки широких
                                      // символов
  s21_size_t size = 0;
  while (arg_s_l[size]) {
    ++size;
...
void write_percent(char *str, int *n, struct_format form) {
  ...
  if (form.minus == 1) {
    ...
    write_space(str, n, length,
                form);  // нет минуса, но есть ширина, то тогда сначала печатаем
                        // пробелы, потом символ
    str[*n] = '%';
    (*n)++;
  }
}
